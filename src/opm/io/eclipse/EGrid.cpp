    ...
    EclFile(filename), inputFileName { filename }, m_grid_name {grid_name}
{
    ...
    for (size_t n = 0; n < array_name.size(); n++) {
        ...
        //std::cout << std::setw(3) << n << " name: " << std::setw(8) << array_name[n] << " grid: '" << std::setw(8) << lgrname;
        //std::cout << "' size: " << array_size[n] << std::endl;
        ...
        if (lgrname == grid_name) {
            ...
            if (array_name[n] == "COORD")
                coord_array_index= n;
            if (array_name[n] == "ZCORN")
                zcorn_array_index= n;
            if (array_name[n] == "ACTNUM")
                actnum_array_index= n;
            if (array_name[n] == "NNC1")
                nnc1_array_index= n;
            if (array_name[n] == "NNC2")
                nnc2_array_index= n;
            if (array_name[n] == "HOSTNUM")
                hostnum_index= n;
        }
...
std::vector<NNCentry> EGrid::get_nnc_ijk() const
{
    if (!m_nncs_loaded)
        load_nnc_data();
...
void EGrid::load_nnc_data() const
{
    if ((nnc1_array_index > -1) && (nnc2_array_index > -1)) {
        ...
        if ((Opm::filesystem::exists(initFileName)) && (nnc1_array.size() > 0)){
            ...
            if ((init_dims != nijk) or (init_nactive != nactive)){
                OPM_THROW(std::invalid_argument, "inconsistent grid dimension in init file");
            }

            auto trans_data = init.getInitData<float>("TRANNNC", m_grid_name);

            if (trans_data.size() != nnc1_array.size())
                OPM_THROW(std::invalid_argument, "inconsistent size of array TRANNNC in init file");
