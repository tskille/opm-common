...
EInit::EInit(const std::string &filename) : EclFile(filename)
{
    ...
    for (size_t n = 0; n < array_name.size(); n++) {
        ...
        //std::cout << " grid '" << std::setw(7) << lgrname << "'";
        //std::cout << " name: " << array_name[n] << std::endl;
...
int EInit::get_array_index(const std::string& name, const std::string& grid_name) const
{
    if (grid_name == "global"){
        if (global_array_index.count(name) == 0)
            OPM_THROW(std::runtime_error, "Map key not found in global_array_index");
        ...
        if (lgr_array_index[lgr_index].count(name) == 0)
            OPM_THROW(std::invalid_argument, "Map key not found in lgr_array_index");
...
{
    if (std::find(lgr_names.begin(), lgr_names.end(), grid_name) == lgr_names.end()) {
