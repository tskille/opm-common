...
public:
    ...
    void load_grid_data() const;
    void load_nnc_data() const;
    ...
    std::vector<NNCentry> get_nnc_ijk() const;
...
private:
    ...
    mutable std::vector<float> coord_array;
    mutable std::vector<float> zcorn_array;

    mutable std::vector<int> nnc1_array;
    mutable std::vector<int> nnc2_array;
    mutable std::vector<float> transnnc_array;
    mutable std::vector<int> host_cells;
    ...
    //template <typename T>
    //std::vector<T> load_from_disk(int array_index) const;
