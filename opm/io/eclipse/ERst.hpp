...
public:
    ...
    void loadReportStepNumber(int number) const;

    template <typename T>
    const std::vector<T>& getRestartData(const std::string& name, int reportStepNumber) const
    {
        return getRestartData<T>(name,reportStepNumber, 0);
    }

    template <typename T>
    const std::vector<T>& getRestartData(const std::string& name, int reportStepNumber, int occurrence) const;

    template <typename T>
    const std::vector<T>& getRestartData(const std::string& name, int reportStepNumber, const std::string& lgr_name) const;

    template <typename T>
    const std::vector<T>& getRestartData(int index, int reportStepNumber) const
    {
    ...
    template <typename T>
    const std::vector<T>& getRestartData(int index, int reportStepNumber, const std::string& lgr_name) const
    {
        auto indRange = this->getIndexRange(reportStepNumber);
    ...
    std::vector<EclEntry> listOfRstArrays(int reportStepNumber) const;
    std::vector<EclEntry> listOfRstArrays(int reportStepNumber, const std::string& lgr_name) const;
...
private:
    ...
    int get_start_index_lgrname(int number, const std::string& lgr_name) const;

    int getArrayIndex(const std::string& name, int seqnum, int occurrence) const;
    int getArrayIndex(const std::string& name, int number, const std::string& lgr_name) const;
